...
export const Alert: React.FC<AlertProps> = ({ alert, onUpdate }) => {
  ...
  const reactionCounts = alert.reactions?.reduce((acc, reaction) => {
    acc[reaction.emoji] = (acc[reaction.emoji] || 0) + 1;
    return acc;
  }, {} as Record<string, number>) || {};

  // Check if current user has already reacted
  const hasUserReacted = user && alert.reactions?.some(reaction => reaction.user.id === user.id);

  // Count total comments (including replies)
  const totalComments = alert.comments?.reduce((total, comment) => {
  ...
  const handleReactionClick = async (emoji: string) => {
    ...
    try {
      await spacesApi.createReaction({
        emoji,
        alertId: alert.id, // Fixed: Use alertId instead of articleId
      });
    ...
    if (commentText.trim() && !isLoading) {
      ...
      try {
        await spacesApi.createComment({
          text: commentText,
          articleId: alert.id, // Using alert.id as the content ID
        });
        setCommentText('');
